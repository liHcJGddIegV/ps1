            ...
            if (![string]::IsNullOrWhiteSpace($EmailAddress)) {
                if ($EmailAddress -match "@(.+)$") {
                    ...
                    if ($CompanyMapping.ContainsKey($domain)) {
                        ...
                        if ($VerboseOutput) {
                            Write-Host "Setting company for: $($Contact.FullName) (Field: $field, Email: $EmailAddress) to 'To Review'"
                        }
                        if (-not $DryRun) {
                        ...
                        $updatedCount++
                        $companyUpdated = $true
                        Write-Host "[OK] Updated: '$($Contact.FullName)' -> Company set to 'To Review'" -ForegroundColor Green
                        break
                    }
                }
                else {
